/* linux/drivers/input/touchscreen/mini2440_tc.c
 *	Copyright (c) 2013 Feng Guoqing
 *
 * mini2440 tochscreen Driver
 *
...
static char *s3c2410ts_name = "mini2440 TouchScreen";
...
static int __init s3c2410ts_init(void)
{
    ...
    /* Get irqs */
    if (request_irq(IRQ_ADC, stylus_action, IRQF_SHARED, "s3c2410_action", dev)) {
        printk(KERN_ERR "mini2440_ts.c: Could not allocate ts IRQ_ADC !\n");
        iounmap(base_addr);
        return -EIO;
    }
    if (request_irq(IRQ_TC, stylus_updown, 0, "s3c2410_action", dev)) {
        printk(KERN_ERR "mini2440_ts.c: Could not allocate ts IRQ_TC !\n");
        iounmap(base_addr);
        return -EIO;
    }
