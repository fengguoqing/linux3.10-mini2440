/* linux/drivers/char/mini2440_pwm.c
...
*/ 
static void PWM_Set_Freq( unsigned long freq ) //??pwm???,???????
{ 
    unsigned long tcon; 
    unsigned long tcnt; 
    unsigned long tcfg1; 
...
{ 
    switch (cmd) { 
        case PWM_IOCTL_SET_FREQ: //if cmd=1 ???case PWM_IOCTL_SET_FREQ 
            if (arg == 0)
                return -EINVAL;
            PWM_Set_Freq(arg);
            break; 
        case PWM_IOCTL_STOP:  // if cmd=2 case PWM_IOCTL_STOP
            PWM_Stop();
            break; 
    } 
    return 0;
